  ...
  return (
    <div className="min-h-screen bg-[#020202] text-[#737373] p-8 flex flex-col justify-between text-[11px] uppercase tracking-wider font-mono">
      <div className="w-full flex gap-6 items-start">
        ...
        {/* Main Board Kanban Stream */}
        <div className={`space-y-6 flex-1 transition-all duration-300 ${sel ? 'max-w-[70%]' : 'w-full'}`}>
            ...
            <div className="flex items-center gap-2">
              <div className="flex gap-1 bg-[#09090c] p-1 rounded-lg border border-[#14141a]">
                {clients.map(c => (
                  ...
                  >
                    {c.key === 'rainmaker' ? 'MINE' : c.label.split(' ')}
                  </button>
                ))}
              </div>
        ...
        {/* 🛠️ Dynamic Interactive Proposal Builder Drawer Sidebar */}
        {sel && (
          <div className="w-[30%] bg-[#060609] border border-[#181824] rounded-lg p-4 space-y-4 sticky top-8">
            <div className="flex justify-between items-center border-b border-[#1b1b26] pb-3">
              <div>
                <span className="text-[9px] text-indigo-400 font-bold">PROPOSAL BUILDER</span>
                <h3 className="text-neutral-200 font-bold text-[12px] truncate max-w-[180px]">{sel.company}</h3>
              </div>
              <button onClick={() => setSel(null)} className="text-[#404040] hover:text-neutral-200 font-sans font-bold text-sm cursor-pointer">✕</button>
            </div>

            {/* Editable Proposal Variables & Specs */}
            <div className="space-y-3">
              <div className="space-y-1">
                <label className="text-[9px] text-[#404040] font-bold block">DEAL VALUE PROJECTION ($)</label>
                <input 
                  type="number" 
                  value={sel.value || ''} 
                  onChange={(e) => setField(sel.id, 'value', Number(e.target.value))}
                  className="w-full bg-[#0d0d12] border border-[#1b1b24] p-2 rounded text-neutral-200 font-mono text-[10px] outline-none" 
                />
              </div>

              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1">
                  <label className="text-[9px] text-[#404040] font-bold block">UNIT VOLUME (QTY1)</label>
                  <input 
                    type="number" 
                    value={sel.qty1 || ''} 
                    onChange={(e) => setField(sel.id, 'qty1', Number(e.target.value))}
                    className="w-full bg-[#0d0d12] border border-[#1b1b24] p-2 rounded text-neutral-200 font-mono text-[10px] outline-none" 
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] text-[#404040] font-bold block">ADD-ON CORES (QTY2)</label>
                  <input 
                    type="number" 
                    value={sel.qty2 || ''} 
                    onChange={(e) => setField(sel.id, 'qty2', Number(e.target.value))}
                    className="w-full bg-[#0d0d12] border border-[#1b1b24] p-2 rounded text-neutral-200 font-mono text-[10px] outline-none" 
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                <label className="text-[9px] text-[#404040] font-bold block">PIPELINE MILESTONE STATUS</label>
                <select 
                  value={sel.status} 
                  onChange={(e) => setField(sel.id, 'status', e.target.value as any)}
                  className="w-full bg-[#0d0d12] border border-[#1b1b24] p-2 rounded text-neutral-300 font-mono text-[10px] outline-none uppercase cursor-pointer"
                >
                  <option value="qualifying">Qualifying</option>
                  <option value="proposal">Proposal Stage</option>
                  <option value="secured">Secured Deal</option>
                  <option value="lost">Lost Lead</option>
                </select>
              </div>

              {/* Account Dossier Sheet */}
              <div className="bg-[#0b0b11] border border-[#14141e] p-2.5 rounded text-[9px] text-[#525252] space-y-1 normal-case tracking-normal font-sans mt-2">
                <span className="font-mono text-[8px] font-bold uppercase tracking-wider text-[#404040] block mb-1">Account Dossier Sheet</span>
                <div>🏠 <span className="font-mono uppercase text-[9px] text-neutral-400">{sel.address}</span></div>
                <div>✉️ <span className="text-neutral-400">{sel.email}</span></div>
                <div>📞 <span className="font-mono text-neutral-400">{sel.phone}</span></div>
              </div>
            </div>

            <div className="pt-2 border-t border-[#1b1b26] flex flex-col gap-1.5">
              <div className="text-[9px] text-[#404040] font-bold">ESTIMATED STAGE PROBABILITY RETENTION:</div>
              <div className="text-sm font-bold text-neutral-300 font-mono">
                ${((sel.value || 0) * (weights[sel.status] ?? 0)).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                <span className="text-[8px] text-neutral-600 font-sans font-normal italic ml-1 lowercase">({(weights[sel.status] ?? 0) * 100}% stage weight ratio applied)</span>
              </div>
            </div>
